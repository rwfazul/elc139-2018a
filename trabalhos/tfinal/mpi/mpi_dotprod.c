...
double dotprod_mpi(int wsize, int repeat, int myrank, int nproc);
...
int main(int argc, char **argv) {
    ...
    if (myrank != 0) { 	/* SLAVES */
        result = dotprod_mpi(wsize, repeat, myrank, p);
        // envia mensagem ao processo 0
        MPI_Send(&result, 1, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD); // 1 = qtd elementos
    } else {            /* MASTER */
        double total_dotprod = dotprod_mpi(wsize, repeat, myrank, p); // 'master' tambem realiza o calculo
...
double dotprod_mpi(int wsize, int repeat, int myrank, int nproc) {
    ...
    double *a = dotdata.a;
    double *b = dotdata.b;  
    int slice = (int) wsize / nproc;
    int start = myrank * slice;
    int end = (myrank < nproc - 1) ? start + slice : wsize;
    double mysum;
...
void show_info(double total_dotprod, int wsize, int p, long start_time, long end_time, double start_time_mpi, double end_time_mpi) {
    ...
    printf("number of processes: %d, wall time: %ld usec, ", p, (long) (end_time - start_time));
...
void check_result(double total_dotprod, int wsize, int qtd_proc) {
    ...
    double expected_result =  VALUE_ARRAY_A * VALUE_ARRAY_B * wsize; 
