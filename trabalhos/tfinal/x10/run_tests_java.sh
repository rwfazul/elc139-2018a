...
RunTests () {
    ...
    time_finish=0
    output_file=$PROGRAM"_java-$1-$2-$3.out"
	
    for (( i = $start; i < $end; i++ )) 
    do
        bin/x10 $PROGRAM $1 $2 $3 >> $output_file
...
if [ $ARGC -ne $MAX_ARGS ]; then 
    echo -e "Uso: $0 <nAct> <wsize> <repeat>\n">&2 #testes param.
else
    # ./compile_java.sh
    RunTests $1 $2 $3 # $1 = activities, $2 = worksize, $3 = repetitions
fi
